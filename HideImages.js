...
(function HideImages()
{
	...
	function artistPageDiscoveryImageObserver()
	{
		const observer = new MutationObserver((mutations) =>
		{
			...
			// const uri = "spotify:artist:" + location.pathname.replace('/artist/', ''); -> we dont actually need this, and btw, this replacement is wrong, because it may end in .../discography/2133
	...
	// First, I tried escaping with https://www.freeformatter.com/javascript-escape.html
	// Escaping 's manually gave me odd errors (the last 20% of the string was lost).
	// But for some reason it still didn't work. I have no idea why.
	// Update: don't escape anything, just use backticks for the string literal. Now it works.
	// The .json file is always updated first, this string second. This way there will be no confusion which one is newer.
	...
	// docs: https://spicetify.app/docs/development/api-wrapper/classes/context-menu/
	...
	new Spicetify.ContextMenu.Item(
		...
		([uri]) =>
		{
			...
			switch (type)
			{
				...
				// LOCAL_ALBUM
				// LOCAL_ARTIST
				// more types at: https://spicetify.app/docs/development/api-wrapper/types/uri/type/
			}
			return false;
		},
		trashbinIcon,
	...
	function watchChange()
	{
		...
		//if (!track) return;
		let albumUri = data.item.metadata.album_uri;
		//let albumUri2 = contextTrack.metadata["album_uri"];
		//console.log (albumUri);
		//console.log (albumUri2);
		let isBannedAlbum = trashAlbumList.has(albumUri);

		const isBanned = trashSongList[data.item.uri] || isBannedAlbum;
